...
\section{Application and results}
	...
	\subsection{Results and comparison }
		...
		General remark on hyperparameter optimization for neural networks: Since neural network models tends to have a large set of hyperparameters, from number of neurons, regularization, learning rate etc, and take a fairly long time to train, hyperparameter tuning can take a long time. Possible remedies to this is to use multiple computers in a cloud environment and train many networks in parallel across different configurations. With domain expertise, there is also possible to limit the search space.\\\\
		We use a grid search approach to determine the hyperparameter settings.\\ For the Franke data, we see that a learning rate of $0.1$ to $0.01$ performs best. There is little difference in the error for changes in lambda, but the best result is achieved at $\lambda$ $0.1$.
		\begin{table}[H]
